...
public static partial class SightLogic
{
    ...
    public static List<SightItem> ImportSvg(string path)
    {
        ...
        foreach (var e in XDocument.Load(path, LoadOptions.None).Descendants())
        {
            ...
            {
                case "line": items.Add(new(SightItemKind.Line,A("x1"),A("y1"),A("x2"),A("y2"))); break;
                case "rect": items.Add(new(SightItemKind.Rectangle,A("x"),A("y"),A("width"),A("height"))); break;
                case "circle": items.Add(new(SightItemKind.Ellipse,A("cx"),A("cy"),A("r",1),A("r",1))); break;
                case "ellipse": items.Add(new(SightItemKind.Ellipse,A("cx"),A("cy"),A("rx",1),A("ry",1))); break;
                case "polyline":
                case "polygon":
                    var nums = NumberRegex().Matches(e.Attribute("points")?.Value ?? "").Select(m => double.Parse(m.Value, CultureInfo.InvariantCulture)).ToArray();
                    var pts = nums.Chunk(2).Where(p => p.Length == 2).Select(p => new PointF((float)p[0],(float)p[1])).ToList();
                    for (var i=1;i<pts.Count;i++) items.Add(new(SightItemKind.Line,pts[i-1].X,pts[i-1].Y,pts[i].X,pts[i].Y));
    ...
    private static IEnumerable<SightItem> PathLines(string? value)
    {
        ...
        while (position < tokens.Length)
        {
            ...
            {
                var nextX = Next(); var nextY = Next();
                if (relative) { nextX += x; nextY += y; }
                x = startX = nextX; y = startY = nextY;
            ...
            else if (upper == 'L' && position + 1 < tokens.Length)
            {
                var nextX = Next(); var nextY = Next();
                if (relative) { nextX += x; nextY += y; }
                result.Add(new(SightItemKind.Line, x, y, nextX, nextY)); x = nextX; y = nextY;
            }
            else if (upper == 'H' && position < tokens.Length)
            {
                var nextX = Next(); if (relative) nextX += x;
                result.Add(new(SightItemKind.Line, x, y, nextX, y)); x = nextX;
            }
            else if (upper == 'V' && position < tokens.Length)
            {
                var nextY = Next(); if (relative) nextY += y;
                result.Add(new(SightItemKind.Line, x, y, x, nextY)); y = nextY;
            }
            else if (upper == 'Z')
            {
    ...
    {
        var left=items.Min(i=>i.Bounds.Left); var top=items.Min(i=>i.Bounds.Top);
        var right=items.Max(i=>i.Bounds.Right); var bottom=items.Max(i=>i.Bounds.Bottom);
        var scale=32/Math.Max(.001,Math.Max(right-left,bottom-top)); var cx=(left+right)/2; var cy=(top+bottom)/2;
        return items.Select(i=>i.Kind switch {
            SightItemKind.Line=>new SightItem(i.Kind,(i.X1-cx)*scale,(i.Y1-cy)*scale,(i.X2-cx)*scale,(i.Y2-cy)*scale),
            SightItemKind.Ellipse=>new SightItem(i.Kind,(i.X1-cx)*scale,(i.Y1-cy)*scale,Math.Abs(i.X2*scale),Math.Abs(i.Y2*scale)),
            _=>new SightItem(i.Kind,(i.X1-cx)*scale,(i.Y1-cy)*scale,i.X2*scale,i.Y2*scale)
        }).ToList();
    }
    ...
    public static string Compact(string text)
    {
        ...
        {
            var values=NumberRegex().Matches(m.Groups[2].Value).Select(n=>Number(double.Parse(n.Value,CultureInfo.InvariantCulture)));
    ...
    public static string ReplaceSightFunction(string source,string mode0,string mode1,Color color)
    {
        ...
        var brace=source.IndexOf('{',start); var depth=0; var end=-1;
        for(var i=brace;i<source.Length;i++){if(source[i]=='{')depth++;else if(source[i]=='}'&&--depth==0){end=i+1;break;}}
